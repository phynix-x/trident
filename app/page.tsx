...
export default function Home() {
  ...
  return <main>
    <header className="header">
      <a className="brand" href="#top"><span>TT</span> TRIDENT</a>
      <nav>
        <a href="#top">Home</a>
        <div className="nav-drop"><a href="#equipment">Equipment ▾</a><div className="drop-menu"><b>Equipment</b>{categoryNames.slice(1).map(c => <button key={c} onClick={() => {setFilter(c); document.getElementById('equipment')?.scrollIntoView()}}>{c}</button>)}</div></div>
    ...
    <section id="industries" className="section"><p className="eyebrow">INDUSTRIES WE SERVE</p><h2>Built around project requirements.</h2><div className="industry-grid">{industries.map((x,i)=><div key={x}><small>0{i+1}</small><strong>{x}</strong><span>Explore solutions →</span></div>)}</div></section>

    <section id="locations" className="location-band"><div><p className="eyebrow">LOCATIONS</p><h2>Equipment support where your project needs it.</h2><p>Tell us your project location and requirement. Trident will confirm suitable equipment and deployment options.</p></div><button className="primary" onClick={()=>setQuoteOpen(true)}>Check Availability</button></section>
    ...
    <section id="contact" className="contact"><div><p className="eyebrow">START A PROJECT</p><h2>Tell us what you need.</h2><p>Submit your requirement and our team will confirm equipment availability for your location and dates.</p></div><button className="primary" onClick={()=>setQuoteOpen(true)}>Request a Quote →</button></section>
